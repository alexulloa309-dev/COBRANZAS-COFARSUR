...
export async function GET() {
  try {
    // Buscar vencimientos con mora > 30 días, con info del cliente
    ...
    vencimientos?.forEach((venc: any) => {
      ...
      // Chequear si el cliente ya está en la lista de la cadena
    ...
    console.error('Error fetching legales:', error);
    return NextResponse.json({ error: 'Error fetching legales', details: error.message }, { status: 500 });
  }
}
